...
public class OrderDetailController {
	...
	public ResponseEntity<?> addCartWithBook(@RequestBody HashMap<String, String> addCartRequest) {
		...
		try {
			...
			List<OrderDetailDTO> obj = orderDetailService.addCartByUserIdAndProductId(id_book, id_user, quantity,
					total);
	...
	public ResponseEntity<?> removeCartWithIdBook(@RequestBody HashMap<String, String> removeCartRequest) {
		try {
			...
			List<OrderDetailDTO> obj = orderDetailService.removeCartByUserId(Long.parseLong("id_cart"),
					Long.parseLong("id_user"));
			return ResponseEntity.ok(obj);
		} catch (Exception e) {
			return ResponseEntity.badRequest().body(new BaseResponse(false, e.getMessage()));
		}
